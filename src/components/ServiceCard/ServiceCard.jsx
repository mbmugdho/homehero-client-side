...
const ServiceCard = ({ service }) => {
  ...
  const serviceId =
    (_id && _id.toString ? _id.toString() : _id) ||
  ...
  const isOwner =
    ...
    (providerEmail &&
      user?.email &&
      String(providerEmail).toLowerCase() === String(user.email).toLowerCase())
    ...
    >
      <figure className="relative h-44 md:h-48 overflow-hidden rounded-xl">
        ...
        <div className="absolute left-3 top-3 flex gap-2">
          ...
          {isOwner && (
            <span className="badge cosmic-badge text-white">Your service</span>
          )}
        </div>
      </figure>
      ...
      <div className="card-body p-4">
        ...
        <div className="card-actions mt-4 gap-2">
          ...
          <button
            ...
            className="cosmic-btn-outline disabled:opacity-60 disabled:cursor-not-allowed"
          >
            Add Service
          </button>
        </div>
